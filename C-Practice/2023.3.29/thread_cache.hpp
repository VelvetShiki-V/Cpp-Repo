...
public:
	...
	void* allocate(const size_t& size){
		...
		if (fl[memory_locate].empty()) { 
			// 到指定桶上获取所需内存
			logMessage(DEBUG, "thread cache[%d]桶内结点为空，开始向central cache索要cache", memory_locate);
	...
	void* fetch_from_central_cache(const size_t& size, const size_t& location) {
		...
		else fl[location].push_batch(next_node(start), end, actual_fetch - 1);
		return start;
	}
	...
	void deallocate(void* ptr, const size_t& size) {
		...
		logMessage(DEBUG, "线程申请%d字节，实际申请%d字节cache已归还至%d号桶上，归还后该桶剩余cache个数: %d个", size, Alignment::space_round_up(size), memory_pos, fl[memory_pos].freelist_size());
		// 当链表结点挂接过长时，需要还给central cache（过长判断：结点挂接数量超过批量申请的数量）
		if (fl[memory_pos].freelist_size() >= fl[memory_pos].max_nodes()) {
			logMessage(DEBUG, "[%d]桶当前挂接结点数: %d个，已达到一次批量申请结点个数上限: %d个，开始将批量数个结点归还至central cache", memory_pos, fl[memory_pos].freelist_size(),fl[memory_pos].max_nodes());
	...
	void freelist_payback(FreeList& fl, const size_t& nodeNums, const size_t& pos) {
		...
		logMessage(DEBUG, "地址从%p--%p的一次性批量申请内存块共: %d个cache从thread cache[%d]桶中取出，准备归还central cache的span中",
			begin, end, nodeNums, pos);
		CentralCache::get_cenctral_cache()->cache_to_span(begin, nodeNums, pos);
	}
private:
