...
public:
	...
	Span* get_span(SpanList& sl, const size_t& size) {			// size用于计算所需申请的page页桶
		...
		logMessage(DEBUG, "获取到一个大内存span地址为: %p, 页号: %d, 页数: %d, \
					对应内存块首地址: %p", span, span->page_id, span->page_nums, start);
		...
		if (sp.isEmpty()) {
			...
			while (start != sp.end()) {
				if (start->freelist == nullptr) {
					...
					logMessage(DEBUG, "span check发现桶有剩余可用span结点，地址: %p", start);
					return start;
				}
			}
		}
