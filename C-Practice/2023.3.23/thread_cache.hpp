...
public:
	...
	void* allocate(const size_t& size){
		...
		if (fl[memory_locate].empty()) { 
			...
			logMessage(DEBUG, "线程%d--thread cache中%d桶原为空，现已向central cache获取内存成功，\
			返回结点首地址: %p", std::this_thread::get_id(), memory_locate, newMemory);
			return newMemory;
		}
		else {
			// 3. 否则直接取得内存块
			logMessage(DEBUG, "%d桶不为空, 获取threac cache成功", memory_locate);
	...
	void* fetch_from_central_cache(const size_t& size, const size_t& location) {
		...
		size_t fetch_num = std::min(max, fetch_limit);
		...
		logMessage(DEBUG, "实际获得内存块%d个，start: %p, end: %p", actual_fetch, start, end);
