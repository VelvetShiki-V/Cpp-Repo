...
namespace HashBucket {
	...
	struct __hash_iterator {
		...
		typedef __hash_iterator<K, T, Hash, KeyInT> iterator;
		...
		iterator& operator++() {
			...
			if (cur) {			// 当前桶迭代
				...
				++location;
				while (location < m_table->max_size() && !m_table->m_buckets[location]) ++location;
				if (location == m_table->max_size()) {		// 如果哈希表找完都没遇到下一个桶，则结束
					m_node = nullptr;
					return *this;
				}
				m_node = m_table->m_buckets[location];		// 如果找到桶，返回该桶首元素迭代器
				return *this;
			}
		}
		T& operator*() {
		...
		bool operator!=(iterator obj) const {	// 不能写为iterator& 
			return m_node != obj.m_node;
		}
		bool operator==(iterator obj) const {
			return m_node == obj.m_node;
		}
	};
	...
	public:
		typedef HashNode<T> Node;
		typedef __hash_iterator<K, T, Hash, KeyInT> iterator;
		friend iterator;		 // 迭代器必须声明为友元类，否则迭代器无法获取哈希表数组
		...
		iterator begin() {
			...
			if (location == max_size()) {			// 如果哈希表找完都没遇到下一个桶，则返回空迭代器
				return end();
			}
			return iterator(m_buckets[location], this);		// 如果找到就将第一个桶的首结点地址构造匿名迭代器返回
		}
		iterator end() {
			return iterator(nullptr, this);					// this指代当前所使用的哈希表
		}
		// 哈希表插入
		pair<iterator, bool> insert(const T& data) {
			Hash transfer;
			KeyInT type;
			iterator fd = find(type(data));
			// 去重——冗余值不插入
			if (fd.operator!=(end())) {		// 迭代器不为空，说明找到冗余值，返回pair
		...
		iterator find(const K& key) {
			if (m_buckets.size() == 0) return end();
			Hash transfer;
			KeyInT type;
