...
public:
	...
	Span* get_span(SpanList& sl, const size_t& size) {			// size用于计算所需申请的page页桶
		...
		logMessage(DEBUG, "页数为%d，共切分了size为: %d的%d个cache小块",span->page_nums, size, span->node_counts);
		...
		if (sp.isEmpty()) {
			...
			while (start != sp.end()) {
				if (start->freelist == nullptr) {
					...
					logMessage(NORMAL, "span check发现桶有剩余可用span结点，该size为:%d的span内存在%d个剩余cache，首地址为: %p", size, start->node_counts, start);
					return start;
				}
			}
		}
	...
	size_t fetch_blocks(void*& start, void*& end, const size_t& batch_num,const size_t& size) {
		...
		span->use_count += actual_get;
		span->node_counts -= actual_get;		// DEBUG，监视span内剩余结点数量
		span->freelist = next_node(end);
		next_node(end) = nullptr;
		logMessage(DEBUG, "thread cache实际获得: %d个，start: %p, end: %p", actual_get, start, end);
	...
	void cache_to_span(void* start, const size_t& size) {
		// 1. 若都还回来了，将整一个span归还给page cache，定位桶，加锁执行
		size_t pos = Alignment::bucket_locate(size);
		sp_list[pos].sl_mtx.lock();
		// 2. 定位span地址
		// 3. 将批量cache头插到该span中
		while (start) {
			...
			span->use_count--;
			if (0 == span->use_count) {
				sp_list[pos].erase(span);			// span清理（页号页数不能清理，暂存至page cache待后续使用）
				span->freelist = nullptr;			
				span->next = span->prev = nullptr;
				span->node_counts = 0;
				sp_list[pos].sl_mtx.unlock();
				logMessage(DEBUG, "thread cache结点全部归还至central cache下[%d]桶地址为%p的span内，将取走该span归还至page cache中" , pos, span);
