...
// 子进程给客户端主机提供数据通信IO服务
static void service(int fd, const std::string& cli_ip, const uint16_t& cli_port, const std::string& name) {
    ...
    while(true) {
        ...
        else if(s == 0) {        // 接收到数据为0，代表对端（写）关闭连接，此时读端也关闭
...
public:
    ...
    void start() {          // 开始服务（多进程版本——父进程监听待通信主机，子进程提供服务）
        ...
        while(true) {
            ...
            // 线程池版本4:
            Task t(service_sock, cli_ip, cli_port, service);        // service会任务执行的回调方法，为客户端提供服务
...
private:
    static void* threadRoutine(void* argss) {
        ...
        service(td->m_sock, td->m_ip, td->m_port, std::to_string(pthread_self()));
        return nullptr;
    }
